...
import { normalizeWord, passesBasicFilter } from "@/lib/wordRules";
...
export async function POST(req: Request) {
  try {
    ...
    const cleaned = normalizeWord(String(word || ""));
    if (!cleaned) {
      return NextResponse.json(
        { ok: false, error: "One word only (no spaces), max 20 chars." },
        { status: 400 }
      );
    }

    if (!passesBasicFilter(cleaned)) {
      return NextResponse.json({ ok: false, error: "That word isn’t allowed." }, { status: 400 });
    }
    ...
    if (!lockActive || room.active_lock_author_id !== authorId) {
      return NextResponse.json({ ok: false, error: "You don’t hold the thread right now." }, { status: 403 });
    }
    ...
    if (author?.last_post_at) {
      ...
      if (secondsSince < COOLDOWN_SECONDS) {
        return NextResponse.json(
          { ok: false, error: `Cooldown: wait ${Math.ceil(COOLDOWN_SECONDS - secondsSince)}s`, cooldown: true },
          { status: 429 }
        );
      }
    }
